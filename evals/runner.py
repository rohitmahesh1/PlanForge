...
def main() -> int:
    ...
    cases = load_cases(repo_root / "evals" / "cases", suite=args.suite)
    if not cases:
        print("No eval cases found.", file=sys.stderr)
        return 1

    adapters = _adapters(repo_root)
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run deterministic PlanForge eval cases.")
    parser.add_argument(
        "--suite",
        default=None,
...
def _adapters(repo_root: Path) -> Dict[str, Callable[[EvalCase], Tuple[dict, EvalMetrics]]]:
